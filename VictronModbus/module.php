...
class VictronModbus extends IPSModule {
    ...
    public function RequestRead() {
        ...
        $this->SendDebug("GetData", $this->GetIDForIdent("GridL1").": ".($GridL1[1] + ($GridL1[2] << 16))/1, 0);
        $Address = 0x335;
        ...
        $this->SendDebug("GetData", $this->GetIDForIdent("GridL2").": ".$GridL2, 0);
        SetValue($this->GetIDForIdent("GridL2"), ($GridL2[1] + ($GridL2[2] << 16))/1);
        $this->SendDebug("GetData", $this->GetIDForIdent("GridL2").": ".($GridL2[1] + ($GridL2[2] << 16))/1, 0);
        $Address = 0x336;
        ...
        $this->SendDebug("GetData", $this->GetIDForIdent("GridL3").": ".$GridL3, 0);
        SetValue($this->GetIDForIdent("GridL3"), ($GridL3[1] + ($GridL3[2] << 16))/1);
        $this->SendDebug("GetData", $this->GetIDForIdent("GridL3").": ".($GridL3[1] + ($GridL3[2] << 16))/1, 0);
